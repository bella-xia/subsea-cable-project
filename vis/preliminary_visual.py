...
def prelim_image_processor(input_dir, start_time, end_time,
                            unit='date', contiguous_flag=False):
    ...
    with gzip.open(input_dir, 'rt', encoding='utf-8') as f:
        for line in f:
    ...
    if contiguous_flag:
        expected_keys = generate_date_sequence(start_time, end_time) 
        expected_rtt_avg, expected_rtt_std = [], []
        expected_num_avg, expected_num_std = [], []
        counter = 0
        while keys[counter] < start_time:
            counter += 1
    ...
    elif start_time == 'xx' and end_time == 'xx':
        expected_keys = keys
        ...
        for k, v1, v2, v3, v4 in zip(keys, max_rtt_avg, max_rtt_std, hop_num_avg, hop_num_std):
            if start_time != 'xx' and start_time > k:
                continue
            if end_time != 'xx' and end_time < k:
                break
    ...
    fig3, axes3 = plt.subplots(2, 1, figsize=(30, 10))
    x_pos = range(len(expected_keys))
    width = 10
    
    bars = axes3[0].bar(x_pos, expected_rtt_avg, yerr=expected_rtt_std)
    for bar in bars:
        height = bar.get_height()
        axes3[0].text(bar.get_x() + bar.get_width() / 2, height + 50, f'{height:.2f}', ha='center', va='bottom', fontsize=8)
    axes3[0].set_xlabel('Time Duration')
    axes3[0].set_ylabel('Last-hop Round-trip time')
    axes3[0].set_xticks(x_pos)
    axes3[0].set_xticklabels(expected_keys)
    axes3[0].tick_params(axis='x', labelrotation=30, labelsize=8)
    bars = axes3[1].bar([p for p in x_pos], expected_num_avg, yerr=expected_num_std)
    for bar in bars:
        height = bar.get_height()
        axes3[1].text(bar.get_x() + bar.get_width() / 2, height + 3, f'{height:.2f}', ha='center', va='bottom', fontsize=8)
    axes3[1].set_xlabel('Time Duration')
    axes3[1].set_ylabel('Number of Hops')
    axes3[1].set_xticks(x_pos)
    axes3[1].set_xticklabels(expected_keys)
    axes3[1].tick_params(axis='x', labelrotation=30, labelsize=8)
...
if __name__ == '__main__':
    ...
    fig1, fig2, fig3 = prelim_image_processor(args.input_dir, args.start_time, args.end_time, unit=args.unit, contiguous_flag=contiguous_flag) 
    ...
    fig3.savefig(f"{args.output_dir}/{output_prefix}_traceroute_preliminary_stat.png")
    plt.close()
