...
def matrix_diff_processor(input_dir, norm='sum'): 
    
    ident_pat = re.compile(r'\d{2}\-\d{2}\-\d{2}')
    graph_data = sorted(os.listdir(input_dir))
    node_set = set()
    ...
    for idx in range(len(graph_data) - 1):
        ...
        G_d1 = nx.read_graphml(os.path.join(input_dir, graph_inst_d1))
        G_d2 = nx.read_graphml(os.path.join(input_dir, graph_inst_d2))
